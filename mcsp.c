...
static struct argp_option options[] = {
    ...
    {"recursion_threshold", 'r', "recursion_threshold", 0, "Specify the maximum allowed number of recursion invocations" },
    {"save_every_seconds", 's', "save_every_seconds", 0, "Specify the time (sec) for periodic output." },
    { 0 }
};
...
static struct {
    ...
    int timeout;
    int arg_num;
    int recursion_threshold;
    int save_every_seconds;
} arguments;
...
static error_t parse_opt (int key, char *arg, struct argp_state *state) {
    switch (key) {
        ...
        case 't':
            ...
            break;
        case 'r':
            arguments.recursion_threshold = std::stoi(arg);
            break;
        case 's':
            arguments.save_every_seconds = std::stoi(arg);
            break;
        case ARGP_KEY_ARG:
...
void save_to_file(std::string prepend, int v, int w){
    std::string full_path = arguments.filename1;
    boost::filesystem::path p(full_path);
    boost::filesystem::path dir = p.parent_path();
    boost::filesystem::path out_path = dir / "log.txt";
    ...
    // <time in ms>
    std::ofstream myfile;
    myfile.open(out_path.string(), std::ios::out | std::ios::app );
    myfile << prepend << ": " << v << ", " << w << std::endl;
    myfile.close();
}
...
cout << "Num donaims " << domains.size() << endl;
    int bound = 0;
        ...
        int current_matching_size)
{
    ...
    for (unsigned int i=0; i<domains.size(); i++) {
        ...
        cout << "*** " << current_matching_size << " " << bd.is_adjacent << endl;
...
int solve(const Graph & g0, const Graph & g1, vector<VtxPair> & incumbent,
        vector<VtxPair> & current, vector<Bidomain> & domains,
        vector<int> & left, vector<int> & right, unsigned int matching_size_goal,
        cxxtimer::Timer &timer, int &global_time,
        const vector<int> &vv0, const vector<int> &vv1)
{
    for (unsigned int j=0; j<incumbent.size(); j++) {
           std::cout << vv0[incumbent[j].v] << ":" << vv1[incumbent[j].w] << "; ";
    }
    std::cout << endl << "===================== " << current_recursion_count << std::endl;
    current_recursion_count++;

    timer.stop();
    if (arguments.save_every_seconds != -1) {
        if (timer.count<std::chrono::milliseconds>() >= 1000 * arguments.save_every_seconds) {
            global_time = global_time + int(timer.count<std::chrono::milliseconds>());
            /* Write the results to file .*/
            // Output directory same as first input file directory.
            std::string full_path = arguments.filename1;
            boost::filesystem::path p(full_path);
            boost::filesystem::path dir = p.parent_path();
            std::string fn = "output_" +
                std::to_string(global_time) + ".csv";
            boost::filesystem::path out_path = dir / fn;
            ...
            for (int i=0; i<g0.n; i++) {
                for (unsigned int j=0; j<incumbent.size(); j++) {
                    if (incumbent[j].v == i) {
                        myfile << vv0[incumbent[j].v] << ":" << vv1[incumbent[j].w];

                        // cout << incumbent[j].v << ":" << incumbent[j].w;
            ...
            myfile << timer.count<std::chrono::milliseconds>() << endl;
            ...
            std::cout << "Periodic saving after " << global_time << " milliseconds." << std::endl;
            timer.reset();


            if (global_time >= arguments.timeout * 1000) {
                return -2; // timeout
            }
        }
    timer.start();
    ...
    if (abort_due_to_timeout)
        return 0;
    if (current_recursion_count >= arguments.recursion_threshold && arguments.recursion_threshold!=0)
        return 0;
    ...
    if (current.size() > incumbent.size()) {
        incumbent = current;
        save_to_file("incumbent", incumbent.size(), -1);
    ...
    for (int i=0; i<=bd.right_len; i++) {
        ...
        if (solve(g0, g1, incumbent, current, new_domains, left, right, matching_size_goal,
        timer, global_time, vv0, vv1) == -2) {
            return -2;
        }
    ...
    if (solve(g0, g1, incumbent, current, domains, left, right, matching_size_goal,
        timer, global_time, vv0, vv1) == -2) {
            return -2;
    }
    return 0;
...
vector<VtxPair> mcs(const Graph & g0, const Graph & g1,
    ...
    for (unsigned int label : g0.label) left_labels.insert(label); // [C, C, N, C, O, C]
    for (unsigned int label : g1.label) right_labels.insert(label); // [C, C, C]
    ...
    std::set_intersection(std::begin(left_labels),
                          ...
                          std::inserter(labels, std::begin(labels))); // [C]

    // Create a bidomain for each label that appears in both graphs
    for (unsigned int label : labels) { // for each node label
        int start_l = left.size();
        int start_r = right.size();

        for (int i=0; i<g0.n; i++)
            if (g0.label[i]==label)
                left.push_back(i); // [0, 1, 3, 5]  <-->  [1, 1, 0, 1, 0, 1]
        for (int i=0; i<g1.n; i++)
            if (g1.label[i]==label)
                right.push_back(i); // [0, 1, 2]    <-->  [1, 1, 1]

        int left_len = left.size() - start_l;
        int right_len = right.size() - start_r;
        // left: [0, 1, 3, 5, ...]
        // right: [0, 1, 2, ...]
        domains.push_back({start_l, start_r, left_len, right_len, false}); // {0, 0, 4, 3, false}
    }
    ...
    if (arguments.big_first) {
        for (int k=0; k<g0.n; k++) {
            ...
            if (solve(g0, g1, incumbent, current, domains_copy, left_copy, right_copy, goal,
                timer, global_time, vv0, vv1) == -2) {
                return incumbent; // timeout happens, so return
            }
        ...
        vector<VtxPair> current;
        if (solve(g0, g1, incumbent, current, domains, left, right, 1, timer,
                  global_time, vv0, vv1) == -2) {
            return incumbent;
        }
    }
